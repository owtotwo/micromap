...
impl<T: PartialEq, const N: usize> Set<T, N> {
    ...
    #[inline]
    #[must_use]
    pub fn into_capacity<const M: usize>(self) -> Set<T, M> {
        assert!(
            self.len() <= M,
            "The new Set<_, M> is too small to hold the data."
        );
        Set {
            map: self.map.into_capacity(),
        }
    }
}
...
mod tests {
    ...
    fn test_into() {
        ...
        let new_capacity_set: Set<i32, 5> = set.into_capacity();
    ...
    fn test_into_panic() {
        ...
        let _small_set: Set<i32, 2> = set.into_capacity();
    }
}
